...
(function (global) {
  ...
  const GROUP_META = {
    1: {
      ...
      decision: "顶层 · 相连 · 白角朝侧面或朝里（不是白朝上分开）",
    },
    2: {
      title: "第 2 组 · 转基本 1/2",
    ...
    4: {
      title: "第 4 组 · 白朝上分开",
      blurb: "角白朝天、两块分开。与第 11 组区别：11 组是相连。",
      decision: "顶层 · 分开 · 白角朝 U（顶面见白）",
    },
    5: {
      title: "第 5 组 · 交叉",
    ...
    11: {
      title: "第 11 组 · 白朝上相连",
      blurb: "角白朝天且与棱相连。与第 4 组关键区别：4 分开、11 相连。",
      decision: "顶层 · 相连 · 白角朝 U（第 4 组是分开）",
    },
    12: {
      title: "第 12 组 · 错槽",
  ...
  /** 组级认图决策树（显示在图鉴顶部） */
  // 实战决策树：先问位置/连接，再问白朝向；空槽多时仍可用理解解，卡壳时对照组号
  const DECISION_TREE = [
    { q: "① 两块都在顶层 · 已相连（组 1 / 11）", a: [
      "先问：对着目标槽了吗？没有 → 先转 U/U' 对齐，别急着拆",
      "白朝侧面或朝里 → 第 1 组，能插就直接插（R U R' / F' U' F 等）",
      "白朝上 → 第 11 组（注意：第 4 组是白朝上但分开）",
      "假配对（颜色连着但插进去会错）→ 先拆开，再当「分开」处理",
      "口诀：能插就插 · 不能插先转 U · 转完还不行再拆",
    ]},
    { q: "② 两块都在顶层 · 分开（组 2–5 / 4）", a: [
      "先问「连不连」，再问白朝向（别一上来只盯白）",
      "交叉相对（对角位置）→ 第 5 组",
      "分开 + 白朝上 → 第 4 组",
      "分开 + 白朝侧面 → 先转 U 把角/棱凑近；目标落成第 1 组那种可插配对 → 第 2/3 组思路",
      "空槽还多：用你的理解配对插入即可；拧两下仍散 → 停手，对照上面三类",
    ]},
    { q: "③ 一块已在槽里（组 6–10 / 12）", a: [
      "先出槽：只动当前槽一侧，出完立刻停手看顶层",
      "出槽后只问一句：转一次 U/U'，角和棱能不能变成「相连且可插」？",
      "能 → 优先转顶层配对，再插入（通常最短）",
      "不能 → 再拆/再取，回到「两块在顶层」那棵树",
      "棱卡在中层 E → 第 6/7/8；角顶棱底 → 第 9；角底棱顶 → 第 10；槽里假插入 → 第 12",
      "口诀：出槽 → 停 → 转 U 能否配对？能就转，不能再处理",
    ]},
    { q: "（备用）两块大致在哪？→ 跳到哪组", a: [
      "都在顶层 U → 看①相连或②分开",
      "一块在中层 E → 第 6 / 7 / 8 组",
